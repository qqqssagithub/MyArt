...
@property (nonatomic) NSMutableArray *tempSongArray;
...
- (void)viewDidAppear:(BOOL)animated{
    ...
    _playButtonView.operation = ^(UIButton *button){
        ...
        if (button.tag == 202) {//上
            ...
            NSInteger currentInex = [weakSelf.playerItems indexOfObject:currentItem];
            if (weakSelf.cycleMode == CirculationModeIsSingleCycle) {
                [weakSelf playAtIndex:currentInex];
                return;
            }
            if (weakSelf.cycleMode == CirculationModeIsRandom) {
                if (weakSelf.IS_SEARCHING) {
                    [weakSelf playAtIndex:currentInex];
                    return;
                }
                [weakSelf playAtIndex:random()%(self.dataSource.count)];
                return;
            }
            if (currentInex == 0) {
                currentInex = weakSelf.playerItems.count - 1;
                [weakSelf playAtIndex:currentInex];
                return;
            }
            currentInex--;
        ...
        if (button.tag == 204) {//下
            ...
            if (currentInex == weakSelf.playerItems.count - 1) {
                currentInex = 0;
                [weakSelf playAtIndex:currentInex];
                return;
            }
...
- (void)addPlayQueue{
    AVURLAsset *asset0 = [AVURLAsset assetWithURL:[NSURL URLWithString:@"http://yinyueshiting.baidu.com/data2/music/259495857/259495857.mp3?xcode=335f54841240419a2b8f63f9ac723f36"]];
    AVPlayerItem *item0 = [[AVPlayerItem alloc]initWithAsset:asset0];
    [item0 addObserver:self
           forKeyPath:@"loadedTimeRanges"
              options:NSKeyValueObservingOptionNew context:nil];
    [self.playerItems addObject:item0];
    ...
    if ([keyPath isEqualToString:@"currentSecond"]) {
        if (((int)self.currentSecond - (int)self.totalSecond == 1.00 || (int)self.currentSecond - (int)self.totalSecond == -1.00) && self.currentSecond != 0) {
            ...
            if (self.cycleMode == CirculationModeIsRandom) {
                ...
                [self playAtIndex:random()%(self.dataSource.count)];
                return;
            }
...
- (void)playAtIndex:(NSInteger)index
{
    ...
    dispatch_async(dispatch_queue_create("lrc", NULL), ^{
        [self currentLrc];
        dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)currentLrc{
    ...
    NSString *lrcStr = [NSString stringWithContentsOfURL:[NSURL URLWithString:url] encoding:NSUTF8StringEncoding error:nil];
